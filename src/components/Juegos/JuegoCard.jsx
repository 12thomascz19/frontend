...
function JuegoCard({ juego, enBibliotecaProp }) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(data.message || "Error al quitar el juego");
    }
    ...
    // Si tienes un prop para actualizar la lista de juegos en el padre
    if (juego.actualizarJuegos) {
      juego.actualizarJuegos(data.biblioteca);
    }
  } catch (error) {
    console.error("Error:", error);
              ...
              {menuOpen && (
                ...
                >
                  {!enBiblioteca ? (
                    ...
                    >
                      <FaPlusCircle className="text-[#00FF88]" /> Agregar a mi
                      biblioteca
                    </button>
                  ) : (
                    <button
                    ...
                    >
                      <FaMinusCircle className="text-red-400" /> Quitar de mi
                      biblioteca
                    </button>
                  )}
                </motion.div>
